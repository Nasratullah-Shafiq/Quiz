	...
	if ($Teacher_Name == "" || $Language == "" || $Email == "" || $Gender == "" || $Mobile_No == "" || $Time == "") {
		...
		exit();
			...
			if($update_row){
				...
				echo "<span class = 'error'> Error: Teacher Details Not Updated !</span>";
				exit();
			}
		}
	}
	...
	if ($Faculty == "" || $Language == "") {
		...
		exit();
			...
			if($update_row){
				...
				echo "<span class = 'error'> Error: Faculty Not Updated !</span>";
				exit();
			}
		}
	}	
...
public function addTeachers($Teacher_Name, $Email, $Language, $Gender, $Mobile_No, $Time){
	...
	// }
		...
		if ($chkresult != false) {
			echo "<span class = 'error'> Email address already exist try new one! </span>";
			exit();	
		}
		else{
			...
			if($inserted_row){
				...
				echo "<span class = 'error'> Error: Teacher Not Added !</span>";
				exit();
			}
		}
	}
...
public function addSubjects($Subject, $Language, $Credit_Hours, $Teacher_ID, $Faculty_ID, $Time, $Status){
	...
	if (filter_var($Credit_Hours, FILTER_VALIDATE_INT, array("options" => array("min_range"=>$min, "max_range"=>$max))) === false) {
	    echo "<span class = 'error'> Invalid Credit Hours value should be in range 1 up to 5 !</span>";
	} 
	else if (filter_var($Time, FILTER_VALIDATE_INT) === false) {
	    echo "<span class = 'error'> Invalid Time value !</span>";
	}
	else{
		$chkquery = "select * from Subject where Subject = '$Subject'";
		$chkresult = $this->db->select($chkquery);
		if ($chkresult != false) {
			echo "<span class = 'error'> Subject already exist try new one! </span>";
			exit();	
		}
		else{
			...
			mysqli_set_charset($this->db->link, 'UTF8');
			if($inserted_row){
				echo "<span class = 'success'> Subject Inserted Successfully !</span>";
				exit();
			}
			else{
				echo "<span class = 'error'> Error: Subject Not Inserted !</span>";
				exit();
			}
		}
	}
