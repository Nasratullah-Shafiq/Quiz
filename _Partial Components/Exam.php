...
class Exam{
	...
	public function getQuestion($Subject_ID){
		$query = "SELECT * from Question where Subject_ID = '".$Subject_ID."' and Language = 'English' and Status = '1' ORDER BY Question_ID DESC limit 12";
		mysqli_set_charset($this->db->link, 'UTF8');
		$getData = $this->db->select($query);
		return  $getData;
	}
	public function getQuizResult($Username){
		$query = "SELECT * from Result  where Username = '$Username'";
		mysqli_set_charset($this->db->link, 'UTF8');
		$getData = $this->db->select($query);
		return  $getData;
	}
	public function getQuizResultByID($Result_ID){
