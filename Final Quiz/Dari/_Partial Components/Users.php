...
public function contact($FullName, $Email, $Phone_No, $Message){
	...
	if ($FullName == "" || $Email == "" || $Phone_No == "" || $Message == "") {
		echo "<span class ='error'> Fields must not be empty ! </span>";
		exit();
	}
	else if(filter_var($Email, FILTER_VALIDATE_EMAIL) === false){
		...
		$chkquery = "select * from Contact where Message = '$Message'";
		$chkresult = $this->db->select($chkquery);
		if ($chkresult != false) {
			echo "<span class = 'error'> Message address already exist! </span>";
			exit();	
		}
		else{
			$query = "insert into Contact(FullName, Email, Phone_No, Message) values('$FullName', '$Email', '$Phone_No', '$Message')";
			$inserted_row = $this->db->insert($query);
			if($inserted_row){
				...
				echo "<span class = 'error'> Error: Message Sending Failed !</span>";
				exit();
			}
		}
	}
...
public function updateProfile($Full_Name, $Username, $Email, $Gender, $Phone_No, $Image, $User_ID){
	...
	if ($Password == "") {
//		echo "//<span class ='error'> لطفا پسورد مناسب بنویسید </span>";
		echo "<div class='alert alert-danger text-right' role='alert' style = 'font-size: 16px;'> لطفا پسورد مناسب بنویسید </div>";
		exit();
	}
			...
			$update_row = $this->db->update($query);
			if($update_row){
				echo "<span class = 'success'> اجرا شد !</span>";
				exit();
			}
			else{
				echo "<span class = 'error'> Error: عملیه اجرا نشد !</span>";
				exit();
			}
		}
	}
